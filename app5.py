...
st.set_page_config(page_title="Simulador de Portafolios Personalizado", layout="wide")

# =========================
# 1) CONFIGURACIÓN Y UTILIDADES
# =========================

# Estructura inicial de portafolios (se usará para inicializar el estado)
...
# INPUTS DE LA BARRA LATERAL (LÍMITES AJUSTADOS)
...
# CORRECCIÓN: fut_years con mínimo 1 y paso 1
...
st.sidebar.markdown("---")
st.sidebar.markdown("### Composición Actual (Pesos Normalizados)")
...
# Preparación de tickers (se usa PORTAFOLIOS_INICIALES para asegurar que todos los tickers sean descargados)
all_tickers = set()
...
for i, (nombre_port, pesos_iniciales) in enumerate(PORTAFOLIOS_INICIALES.items()):
    with tabs[i]:
        ...
        st.markdown("##### ✏️ Personaliza la Composición (Peso en %)")
        ...
        st.info(f"Suma de pesos ingresados: **{total_input_peso:.1f}%**. El código normalizará los pesos para los cálculos.")
        st.markdown("---")

        
        # OBTENER METRICAS DEL PORTAFOLIO CON PESOS ACTUALES
        ...
        # --- MONTE CARLO ---
        with st.spinner(f"Ejecutando {n_sim} simulaciones para {fut_years} años..."):
        ...
        # --- VIZUALIZACIÓN DE MÉTRICAS CLAVE ---
        st.markdown("##### 📊 Resumen de Métricas Clave")
        
        col_m1, col_m2, col_m3 = st.columns(3)
        col_m1.metric("Aportes Totales", f"${total_invertido:,.0f}")
        col_m2.metric("Volatilidad Anual", f"{vol_anual:.2%}")
        col_m3.metric("Ratio Sharpe", f"{sharpe:.2f}")

        st.markdown("##### 🕰️ Backtest Histórico vs. 🔮 Proyección Monte Carlo (Mediana)")
        col_b1, col_b2, col_m4, col_m5 = st.columns(4)
        
        col_b1.metric("Valor Final (Histórico)", f"${valor_final_hist:,.0f}")
        col_b2.metric("CAGR (Histórico)", f"{cagr_hist:.2%}")
        
        col_m4.metric("Valor Final (MC Mediana)", f"${mediana_final_mc:,.0f}")
        col_m5.metric("CAGR (Monte Carlo)", f"{cagr_mc:.2%}")
        
        st.divider()
        
        # GRÁFICOS (BACKTEST Y MONTE CARLO)
        
        # Gráfico Backtest
        st.markdown("##### Backtest Histórico (Valor del Portafolio vs. Aportes)")
        ...
        # Monte Carlo
        st.markdown(f"##### Proyección de Riqueza Futura ({fut_years} años)")
        ...
        col_mc2.success(f"Escenario Base (Mediana): **${mediana_final_mc:,.0f}**")

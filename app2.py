...
# NOTA: Asegúrate de que todos estos Tickers sean válidos y tengan historial en YFinance.
PORTAFOLIOS = {
    "Conservador": {
...
def normalizar(series, invertir=False):
    ...
    if series.empty or series.max() == series.min():
        # Devuelve 0.5 si todos los valores son iguales para una normalización neutra
...
def descargar_datos(tickers, years):
    ...
    # st.toast fue removido de aquí para evitar conflictos de caché
    data = yf.download(tickers, period=f"{years}y", interval="1mo", auto_adjust=True, progress=False)
    
    if isinstance(data.columns, pd.MultiIndex):
        data = data["Close"].copy()
    
    # Filtrado robusto de datos (eliminamos columnas con muchos NaN)
...
def retornos_portafolio(ret_activos, pesos_dict):
    ...
    # CORRECCIÓN: Manejar caso de portafolio sin activos
    if not cols:
...
def mc_trayectorias(ret_activos, pesos_dict, aporte_mensual, n_sim, fut_years):
    ...
    # CORRECCIÓN: Verificar si hay activos. Si no, retornar matriz de ceros.
    ...
    # CORRECCIÓN: Manejo de error para matrices de covarianza singulares
    try:
        r_act = np.random.multivariate_normal(mean=mu, cov=cov, size=(meses, n_sim))
    except np.linalg.LinAlgError:
        st.warning(f"⚠️ Fallo en covarianza para activos: {cols}. Usando matriz diagonal para simulación (precisión reducida).")
...
# CORRECCIÓN: Las variables deben definirse ANTES de usarlas.
...
# Carga de datos
try:
    # CORRECCIÓN: Llama a st.spinner FUERA de la función cacheada.
...
with st.expander("📊 Ver Frontera Eficiente de Markowitz", expanded=True):
    ...
    with col1:
        ...
        # Nube de puntos (coloreada por Ratio Sharpe)
        ...
        # Graficar los portafolios específicos
...
for i, (nombre_port, pesos) in enumerate(PORTAFOLIOS.items()):
    with tabs[i]:
        ...
        # Calcular Backtest y Retornos Anualizados
        ...
        with st.spinner(f"Ejecutando {n_sim} simulaciones..."):
            ...
            # Si mc_vals es una matriz de ceros (error en mc_trayectorias)
            ...
            results_mc_store[nombre_port] = mc_vals[-1, :] # Guardar finales para comparativa
...
with tabs[3]:
    ...
    if not df_radar.empty:
        indicadores = list(df_radar.columns) # ["Retorno", "Volatilidad", "Sharpe"]

...
def retornos_portafolio(ret_activos, pesos_dict):
    ...
    # Normalizar pesos aquí para asegurar que sumen 1
    ...
    # w = w / w.sum() # Ya normalizados arriba
...
# INPUTS DE LA BARRA LATERAL (LÍMITES AUMENTADOS)
...
fut_years = st.sidebar.slider("Años a Proyectar (Monte Carlo)", 5, 50, 20) # Aumentado a 50
n_sim = st.sidebar.slider("Nº Simulaciones MC", 100, 10000, 1000, step=100) # Aumentado a 10000
...
# =========================
# ... (Código de Frontera de Markowitz no cambia sustancialmente, se usa PORTAFOLIOS) ...
...
for i, (nombre_port, pesos) in enumerate(PORTAFOLIOS.items()):
    with tabs[i]:
        ...
        # **NUEVA CARACTERÍSTICA: PERSONALIZACIÓN DE PESOS**
        ...
        for j, (ticker, peso) in enumerate(current_pesos.items()):
            with cols[j]:
                # Usamos el 100% en el input para facilitar la edición
                ...
                # Almacenamos el valor en decimal para el cálculo
                new_pesos[ticker] = peso_porc / 100
        
        # Actualizar el estado de la sesión si los pesos han cambiado
        if new_pesos != current_pesos:
            st.session_state['portafolios_personalizados'][nombre_port] = new_pesos
            # st.rerun() no es necesario aquí gracias a la reactividad de Streamlit
            
        total_input_peso = sum(new_pesos.values()) * 100
        
        st.info(f"Suma de pesos ingresados: **{total_input_peso:.1f}%**. El código normalizará los pesos automáticamente para sumar 100%.")
        st.markdown("---")
